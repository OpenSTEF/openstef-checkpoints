...
"""Turning a traced model into ONNX, and the reduced-precision variants.

The steps that do not depend on which model is exported: the `torch.onnx.export` call,
an int64 fix for `Gather` indices, fp16 conversion, and int8 quantization. This module
imports torch, so it is loaded only when an export runs, not by the CLI or the publish
path.
"""
